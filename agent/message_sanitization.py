...
def _repair_tool_call_arguments(raw_args: str, tool_name: str = "?", *, session_id: str | None = None,
    ...
    # Passes 3-4: strip trailing commas, then close unclosed/misnested structures. Bracket
    # counting is string-aware: delimiters inside string values ({"code": "}"}) are not
    # structure, and closers land in stack order — a truncated {"items": [{"n": 1}, {"n": 2
    # needs "}]}" appended, and a misnested {"a": [{"b": 1}, {"c": 2}} needs "]" inserted
    # before the misplaced "}".
